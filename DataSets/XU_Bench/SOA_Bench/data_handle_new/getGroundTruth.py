...
def process_files(input_dir, output_dir, caption_map_path):
    ...
    for filename in tqdm(pkl_files, desc="处理文件"):
        ...
        with open(input_path, 'rb') as f:
            ...
            print(f"处理文件 {filename}，共有 {len(data_list)} 条数据")
        ...
        for item in data_list:
            ...
            if prompt in prompts_set:
                print(f"警告: 跳过重复的prompt '{prompt[:50]}...'")
                continue
            ...
            if prompt in caption_to_id:
                ...
                if image_id in id_set:
                    print(f"警告: 跳过重复的id '{image_id}'")
                    print(f"对应的prompt是 '{prompt[:50]}...'")
                    continue
                id_set.add(image_id)
...
def main():
    # 配置文件路径
    input_dir = "/home/sxm/flux-workspace/Qwen-Image-Lightning/DataSets/XU_Bench/SOA_Bench/data_handle_new/sampled_captions_512res_clear_layout"
    output_dir = "/home/sxm/flux-workspace/Qwen-Image-Lightning/DataSets/XU_Bench/SOA_Bench/data_handle_new/sampled_captions_512res_ground_truth"
